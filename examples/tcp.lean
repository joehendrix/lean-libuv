...
def read_cb (data : ByteString) : BaseIO Unit := do
  pure ()


def connection_cb (loop : UV.Loop) (server : UV.TCP) : UV.UvIO Unit := do
...
def main : IO Unit := do
  ...
  let addr ← UV.SockAddr.mkIPv4 "0.0.0.0" 12000
  server.bind addr
  server.listen 128 (connection_cb loop server)
